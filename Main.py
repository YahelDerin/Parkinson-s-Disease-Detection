...
def main():
    ...
    while user_want!='Exit':
        ...
        if option=="1":
            ...
            print("\n\nThe dataset is prepare\n")
        if option=="2":
            model= the_Model.parkinson_disease_detection_model(input_shape=(128, 128, 1)) #contains the model
            model.summary()
            hist = model.fit(x_train, y_train, batch_size=20, epochs=40, validation_data=(x_test, y_test)) #the trained model
            Show_Data.loss_and_accuracy_plot(hist)
            model.save('NEW_parkinson_disease_detection.h5') #saves the model
